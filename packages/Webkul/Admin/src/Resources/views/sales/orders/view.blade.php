<x-admin::layouts>
    <x-slot:title>
        @lang('admin::app.sales.orders.view.title', ['order_id' => $o->increment_id])
    </x-slot>

    <!-- Header -->
    <div class="grid">
        <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
            {!! view_render_event('bagisto.admin.sales.order.title.before', ['order' => $o]) !!}
            ...
            <div class="flex items-center gap-2.5">
                ...
                <!-- Order Status -->
                <span class="label-{{ $o->status }} text-sm mx-1.5">
                    @lang("admin::app.sales.orders.view.$o->status")
                </span>
            </div>

            {!! view_render_event('bagisto.admin.sales.order.title.after', ['order' => $o]) !!}
    ...
    <div class="mt-5 flex-wrap items-center justify-between gap-x-1 gap-y-2">
        <div class="flex gap-1.5">
            {!! view_render_event('bagisto.admin.sales.order.page_action.before', ['order' => $o]) !!}

            @if (
                $o->canReorder() &&
                    bouncer()->hasPermission('sales.orders.create') &&
                    core()->getConfigData('sales.order_settings.reorder.admin'))
                <a href="{{ route('admin.sales.orders.reorder', $o->id) }}"
            ...
            @if ($o->canInvoice() && bouncer()->hasPermission('sales.invoices.create') && $o->payment->method !== 'paypal_standard')
                @include('admin::sales.invoices.create')
            @endif

            @if ($o->canShip() && bouncer()->hasPermission('sales.shipments.create'))
                @include('admin::sales.shipments.create')
            @endif

            @if ($o->canRefund() && bouncer()->hasPermission('sales.refunds.create'))
                @include('admin::sales.refunds.create')
            @endif

            @if ($o->canCancel() && bouncer()->hasPermission('sales.orders.cancel'))
                <form method="POST" ref="cancelOrderForm" action="{{ route('admin.sales.orders.cancel', $o->id) }}">
                    @csrf
                </form>
            ...
            {!! view_render_event('bagisto.admin.sales.order.page_action.after', ['order' => $o]) !!}
        </div>
        ...
        <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
            ...
            <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
                {!! view_render_event('bagisto.admin.sales.order.left_component.before', ['order' => $o]) !!}

                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    <div class="flex justify-between p-4">
                        <p class="mb-4 text-base font-semibold text-gray-800 dark:text-white">
                            @lang('Order Items') ({{ count($o->items) }})
                        </p>

                        <p class="text-base font-semibold text-gray-800 dark:text-white">
                            @lang('admin::app.sales.orders.view.grand-total', ['grand_total' => core()->formatBasePrice($o->base_grand_total)])
                        </p>
                    </div>
                    ...
                    <div class="grid">
                        @foreach ($o->items as $item)
                            {!! view_render_event('bagisto.admin.sales.order.list.before', ['order' => $o]) !!}
                            ...
                            {!! view_render_event('bagisto.admin.sales.order.list.after', ['order' => $o]) !!}
                        @endforeach
                    </div>
                        ...
                        <div class="grid max-w-max gap-2 text-sm">
                            ...
                            @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                                <div class="flex w-full justify-between gap-x-5">
                                    ...
                                    <p class="font-semibold !leading-5 text-gray-600 dark:text-gray-300">
                                        {{ core()->formatBasePrice($o->base_sub_total_incl_tax) }}
                                    </p>
                                </div>
                            @elseif (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'both')
                                <div class="flex w-full justify-between gap-x-5">
                                    ...
                                    <p class="font-semibold !leading-5 text-gray-600 dark:text-gray-300">
                                        {{ core()->formatBasePrice($o->base_sub_total) }}
                                    </p>
                                </div>
                            ...
                            @if ($haveStockableItems = $o->haveStockableItems())
                                @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                                    <div class="flex w-full justify-between gap-x-5">
                                        ...
                                        <p class="font-semibold !leading-5 text-gray-600 dark:text-gray-300">
                                            {{ core()->formatBasePrice($o->base_shipping_amount_incl_tax) }}
                                        </p>
                                    </div>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                ...
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    {{ core()->formatBasePrice($o->base_tax_amount) }}
                                </p>
                            </div>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                ...
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    {{ core()->formatBasePrice($o->base_discount_amount) }}
                                </p>
                            </div>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                ...
                                <p class="text-base font-semibold !leading-5 text-gray-800 dark:text-white">
                                    {{ core()->formatBasePrice($o->base_grand_total) }}
                                </p>
                            </div>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                ...
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    {{ core()->formatBasePrice($o->base_grand_total_invoiced) }}
                                </p>
                            </div>
                            ...
                            <div class="flex w-full justify-between gap-x-5">
                                ...
                                <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                    {{ core()->formatBasePrice($o->base_grand_total_refunded) }}
                                </p>
                            </div>
                            ...
                            <div class="flex w-full justify-between gap-x-5 font-semibold">
                                ...
                                @if ($o->status !== 'canceled')
                                    <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                        {{ core()->formatBasePrice($o->base_total_due) }}
                                    </p>
                                @else
                                    <p class="!leading-5 text-gray-600 dark:text-gray-300">
                ...
                <div class="box-shadow rounded bg-white dark:bg-gray-900">
                    ...
                    <x-admin::form action="{{ route('admin.sales.orders.comment', $o->id) }}">
                        <div class="p-4">
                            <div class="mb-2.5">
                                <x-admin::form.control-group>
                                    <x-admin::form.control-group.control type="textarea" id="comment"
                                        name="comment" rules="required" :label="trans('admin::app.sales.orders.view.comments')" :placeholder="trans('admin::app.sales.orders.view.write-your-comment')"
                                        rows="3" />
                    ...
                    <!-- Comment List -->
                    @foreach ($o->comments()->orderBy('id', 'desc')->get() as $comment)
                ...
                {!! view_render_event('bagisto.admin.sales.order.left_component.after', ['order' => $o]) !!}
            </div>

            <!-- Right Component -->
            <div class="flex w-[360px] max-w-full flex-col gap-2 max-sm:w-full">
                {!! view_render_event('bagisto.admin.sales.order.right_component.before', ['order' => $o]) !!}
                ...
                <x-admin::accordion>
                    ...
                    <x-slot:content>
                        <div class="{{ $o->billing_address ? 'pb-4' : '' }}">
                            <div class="flex flex-col gap-1.5">
                                <p class="font-semibold text-gray-800 dark:text-white">
                                    {{ $o->customer_full_name }}
                                </p>

                                {!! view_render_event('bagisto.admin.sales.order.customer_full_name.after', ['order' => $o]) !!}

                                <p class="text-gray-600 dark:text-gray-300">
                                    {{ $o->customer_email }}
                                </p>

                                {!! view_render_event('bagisto.admin.sales.order.customer_email.after', ['order' => $o]) !!}

                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('admin::app.sales.orders.view.customer-group') :
                                    {{ $o->is_guest ? core()->getGuestCustomerGroup()?->name : $o->customer->group->name ?? '' }}
                                </p>

                                {!! view_render_event('bagisto.admin.sales.order.customer_group.after', ['order' => $o]) !!}
                            </div>
                        </div>
                        ...
                        @if ($o->billing_address)
                            ...
                            <div class="{{ $o->shipping_address ? 'pb-4' : '' }}">
                                ...
                                {!! view_render_event('bagisto.admin.sales.order.billing_address.after', ['order' => $o]) !!}
                            </div>
                        @endif
                        ...
                        @if ($o->shipping_address)
                            ...
                            {!! view_render_event('bagisto.admin.sales.order.shipping_address.after', ['order' => $o]) !!}
                        @endif
                    </x-slot>
                </x-admin::accordion>
                ...
                <x-admin::accordion>
                    ...
                    <x-slot:content>
                        <div class="flex w-full justify-start gap-5">
                            ...
                            <div class="flex flex-col gap-y-1.5">
                                {!! view_render_event('bagisto.admin.sales.order.created_at.before', ['order' => $o]) !!}
                                ...
                                {!! view_render_event('bagisto.admin.sales.order.created_at.after', ['order' => $o]) !!}
                                ...
                                {!! view_render_event('bagisto.admin.sales.order.status_label.after', ['order' => $o]) !!}
                                ...
                                {!! view_render_event('bagisto.admin.sales.order.channel_name.after', ['order' => $o]) !!}
                            </div>
                        </div>
                    </x-slot>
                </x-admin::accordion>
                ...
                <x-admin::accordion>
                    ...
                    <x-slot:content>
                        <div>
                            ...
                            {!! view_render_event('bagisto.admin.sales.order.payment-method.after', ['order' => $o]) !!}
                        </div>
                        ...
                        @if ($o->shipping_address)
                            ...
                            {!! view_render_event('bagisto.admin.sales.order.shipping-method.after', ['order' => $o]) !!}
                        @endif
                    </x-slot>
                </x-admin::accordion>
                ...
                <x-admin::accordion>
                    <x-slot:header>
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('admin::app.sales.orders.view.invoices') ({{ count($o->invoices) }})
                        </p>
                    </x-slot>

                    <x-slot:content>
                        @forelse ($o->invoices as $index => $invoice)
                ...
                <x-admin::accordion>
                    <x-slot:header>
                        <p class="p-2.5 text-base font-semibold text-gray-600 dark:text-gray-300">
                            @lang('admin::app.sales.orders.view.shipments') ({{ count($o->shipments) }})
                        </p>
                    </x-slot>

                    <x-slot:content>
                        @forelse ($o->shipments as $shipment)
                            <div class="grid gap-y-2.5">
                                <div>
                                    <!-- Shipment Id -->
                                    <p class="font-semibold text-gray-800 dark:text-white">
                ...
                {!! view_render_event('bagisto.admin.sales.order.right_component.after', ['order' => $o]) !!}
            </div>
        </div>
    </div>
</x-admin::layouts>
