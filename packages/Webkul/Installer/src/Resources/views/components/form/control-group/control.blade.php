...
@switch($type)
    ...
    @case('select')
        <v-field name="{{ $na }}" v-slot="{ field }"
            {{ $attributes->only(['value', ':value', 'v-model', 'rules', ':rules', 'label', ':label']) }}>
            <select name="{{ $na }}" v-bind="field"
                :class="[errors['{{ $na }}'] ? 'border border-red-500' : '']"
    ...
    @case('checkbox')
        <v-field v-slot="{ field }" name="{{ $na }}" type="checkbox" class="hidden"
            {{ $attributes->only(['value', ':value', 'v-model', 'rules', ':rules', 'label', ':label']) }}>
        ...
        <label class="icon-checkbox-normal peer-checked:icon-checkbox-active cursor-pointer text-2xl peer-checked:text-blue-600"
