                                ...
                                <x-shop::dropdown
                                    ...
                                    <x-slot:menu class="!py-1 max-sm:!py-0">
                                        <x-shop::dropdown.menu.item>
                                            <a href="{{ route('shop.customers.account.addresses.edit', $addr->id) }}">
                                                <p class="w-full">
                                        ...
                                        <x-shop::dropdown.menu.item>
                                            <form method="POST" ref="addressDelete"
                                                action="{{ route('shop.customers.account.addresses.delete', $addr->id) }}">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        ...
                                        @if (!$addr->default_address)
                                            <x-shop::dropdown.menu.item>
                                                <form method="POST" ref="setAsDefault"
                                                    action="{{ route('shop.customers.account.addresses.update.default', $addr->id) }}">
                                                    @method('PATCH')
                                                    @csrf
